...
import { faBell, faMagnifyingGlass, faChartLine, faDrawPolygon, faExpandAlt } from "@fortawesome/free-solid-svg-icons";
...
const DashboardApp = () => {
    const newsItems = [
        { id: 1, title: "New Investment Opportunities in Green Energy", source: "Market Watch", time: "4h ago" },
        { id: 2, title: "Economic Forecast 2024: Global Markets", source: "Business Daily", time: "6h ago" },
        { id: 3, title: "Cryptocurrency Market Shows Strong Recovery", source: "Crypto News", time: "8h ago" },
        { id: 4, title: "Real Estate Market Trends Q1 2024", source: "Property Weekly", time: "12h ago" },
        { id: 5, title: "Emerging Markets: Growth Opportunities", source: "Global Markets", time: "1d ago" },
        { id: 6, title: "Tech Stocks: Q1 Earnings Report", source: "Tech Insider", time: "2d ago" }
    ];
    ...
    const marketItems = [
        { id: 1, symbol: "GTB", name: "Guaranteed Trust Bank", price: "182.63", change: "+1.25%", volume: "62.5M" },
        { id: 2, symbol: "ZNT", name: "Zenith Bank", price: "375.79", change: "+0.89%", volume: "23.1M" },
        { id: 3, symbol: "MTN", name: "MTN", price: "142.65", change: "-0.45%", volume: "18.7M" },
        { id: 4, symbol: "AFR", name: "Afromedia", price: "155.34", change: "+2.11%", volume: "35.2M" },
        { id: 5, symbol: "CON", name: "CONOIL", price: "238.45", change: "-1.23%", volume: "42.8M" },
        { id: 6, symbol: "META", name: "Meta Platforms", price: "378.53", change: "+1.67%", volume: "19.3M" },
    ];
  ...
  // Mini sparkline charts for portfolio
    ...
    return (
        <section style={{padding:".75rem"}} className="flex gap-2 min-h-screen bg-neutral-800 overflow-hidden">
         <SideBar/>
        <div style={{padding:"1rem"}} className="flex flex-col w-full bg-[#1a1a1a] text-white rounded-lg border-[#1a1a1a] min-h-screen shadow-lg border-r border-light gap-8">
            <div className="flex items-center justify-between gap-4 w-full">
                <div className="flex flex-col gap-1.5">
                    <h1 className="text-3xl font-bold">Dashboard</h1>
                    <p>Welcome back, <span className="font-bold text-xl">Inioluwa</span>👋</p>
                    <p style={{fontSize:10}} className=" text-gray-300">Wallet ID: BBB25-FA597-34C76-DBE98-BFC-59</p>
                </div>
                <div className="flex items-center gap-1.5">
                    <FontAwesomeIcon className="text-gray-600" style={{width:"20px", height:"20px"}} icon={faMagnifyingGlass}/>
                    <input
            ...
            <div style={{marginBlock:"1.5rem"}} className="flex justify-between items-center gap-1.5 w-full">
                <div style={{padding:"1rem", width:"250px", height:"160px"}} className="bg-neutral-800 border border-gray-300 rounded-2xl ">
                    <p className="text-sm text-gray-600">Available Balance</p>
                    <div style={{paddingBlock:"10px"}} className="flex items-center gap-2 rounded-xl">
                        <h1 className="text-3xl font-bold">₦30,000.00</h1>
                        <span style={{padding:"5px"}} className="bg-emerald-300 opacity-40 rounded-lg text-white">+0.00%</span>
                    </div>
            ...
            <div className="flex flex-col gap-4">
                <div  className="flex items-center justify-between w-full">
                    <div>
                        ...
                        <button style={{padding:"0.5rem 1.5rem"}}  className="bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-[var(--primary)] transition-colors opacity-25">1 H</button>
                        <button style={{padding:"0.5rem 1.5rem"}}  className="bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-[var(--primary)] transition-colors opacity-25">24 H</button>
                        <button style={{padding:"0.5rem 1.5rem"}}  className="bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-[var(--primary)] transition-colors opacity-25">1 W</button>
                        <button style={{padding:"0.5rem 1.5rem"}}  className="bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-[var(--primary)] transition-colors">1 M</button>
                        </div>
                     </div>
           ...
           <div className="flex grid-cols-2 items-center justify-between gap-4">
                <div style={{padding:"1rem 0" }} className="w-full flex flex-col gap-2 space-y-4 overflow-y-auto">
                {marketItems.map((market) => (
                <div style={{padding:".5rem 0.25rem"}} key={market.id} className="border-b border-gray-100 pb-3 last:border-0 cursor-pointer hover:bg-gray-700 p-2 rounded">
                    <h3 className="font-medium text-sm">{market.name} ({market.symbol})</h3>
                    <div style={{padding:".35rem 0"}} className="flex justify-between mt-1">
                    <span className="text-xs text-gray-500">${market.price}</span>
                    <span className={`text-xs ${market.change.startsWith('+') ? 'text-green-500' : 'text-red-500'}`}>{market.change}</span>
                    </div>
                </div>
                ))}
            </div>
